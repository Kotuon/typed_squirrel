...
SIMDQuaternion& SIMDQuaternion::operator*=( const SIMDQuaternion& rhs ) {
    ...
    SIMDQuaternion result;
    _mm_store_ss( &( a[0] ), rw1 );
    _mm_store_ss( &( a[1] ), ri1 );
    _mm_store_ss( &( a[2] ), rj1 );
    _mm_store_ss( &( a[3] ), rk1 );
...
float SIMDQuaternion::dot( const SIMDQuaternion& other ) const {
    __m128 q0 = _mm_load_ps( a );
    __m128 q1 = _mm_load_ps( other.a );

    __m128 a0 = _mm_mul_ps( q0, q1 );
    __m128 a1 =
        _mm_add_ps( _mm_shuffle_ps( a0, a0, _MM_SHUFFLE( 2, 3, 0, 1 ) ), a0 );
    __m128 a2 =
        _mm_add_ps( _mm_shuffle_ps( a1, a1, _MM_SHUFFLE( 1, 0, 3, 2 ) ), a1 );
...
SIMDQuaternion& SIMDQuaternion::normalize() {
    ...
    __m128 q0 = _mm_load_ps( a );
    __m128 s0 = _mm_set_ps( recpLength, recpLength, recpLength, recpLength );
