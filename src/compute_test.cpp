...
void ParticlesTest::init() {
    ...
    glBufferData( GL_SHADER_STORAGE_BUFFER, NUM_PARTICES * sizeof( vector3 ),
                  NULL, GL_STATIC_DRAW );

    GLint bufMask = GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT;

    posList = ( vector4* )glMapBufferRange( GL_SHADER_STORAGE_BUFFER, 0,
                                            NUM_PARTICES * sizeof( vector4 ),
                                            bufMask );
